...
\subsection{Zgodność z Ogólną Teorią Względności i Mechanizm Higgsa}

\textbf{Twierdzenie o zgodności SHZ-U z OTW:} 
Hamiltonian sieci horyzontów $H_{\text{SHZ}}$ z aksjomatem połowy energii jest równoważny akcji Einsteina-Hilberta w granicy ciągłej, przy czym energia uwalniana przy złączeniu odpowiada efektom brzegowym generującym potencjał skalarny (Higgs).

\bigskip
\textbf{Dowód:} 

\begin{enumerate}[label=\arabic*.]
    \item \textbf{Natura Energii:} Aksjomat definiuje lokalną "energię topologiczną" węzłów sieci, a nie klasyczną masę OTW.
    \item \textbf{Granica Ciągła:} Zgodnie z rozwiązaniem Problemu 1, w granicy kontinuum zachodzi przejście $H_{\text{SHZ}} \to S_{\text{Regge}} \to S_{\text{Einstein-Hilbert}}$. Makroskopowe prawa zachowania ($\nabla_\mu T^{\mu\nu} = 0$) są w pełni spełnione w granicy ciągłej.
    \item \textbf{Transformacja Brakującej Energii:} Z punktu widzenia struktury dyskretnej energia uwalniana przy złączeniu $E_{\text{uwolniona}} = \frac{1}{2}(E_1 + E_2)$ nie "ginie". Transformuje się ona we fluktuacje brzegowe, które na poziomie pól kwantowych przyjmują postać potencjału o kształcie "sombrero" (mexican hat, $\phi^4$).
    \item \textbf{Wartość Oczekiwana Próżni:} Skumulowana, "tracona" w złączeniach mikroskopowych energia sieci generuje Nieznikającą Wartość Oczekiwaną Próżni (VEV) dla pola skalarnego $v \approx 246 \text{ GeV}$, co daje bezpośredni fizyczny mechanizm powstawania mas cząstek elementarnych.
\end{enumerate}

\textbf{Podsumowanie rozwiązania:}
\begin{center}
\begin{tabular}{ll}
\toprule
\textbf{Poziom} & \textbf{Zachowanie energii} \\
\midrule
Sieć dyskretna (mikro) & Aksjomat: $E_{\text{junction}} = \frac{1}{2}(E_1 + E_2)$ \\
Granica ciągła (makro) & OTW: $\nabla_\mu T^{\mu\nu} = 0$ \checkmark \\
Mechanizm & Fluktuacje brzegowe generują potencjał $\phi^4$ \\
Energia "tracona" & Staje się parametrem VEV pola Higgsa \\
\bottomrule
\end{tabular}
\end{center}
...
\textbf{Postulat Kompatybilności Symetrii Cechowania:} Redukcja wymiaru przestrzeni konfiguracji w dyskretnej sieci horyzontów w sposób naturalny generuje fizyczną przestrzeń stopni swobody o pojemności idealnie odpowiadającej grupie cechowania Modelu Standardowego $SU(3) \times SU(2) \times U(1)$.
...
Ale redukcja do fizycznych stopni swobody wymaga uwzględnienia więzów z dynamiki horyzontów. Z aksjomatu połowy energii układ wymusza dodatkową korelację stanów, która nakłada dokładnie 10 więzów energetycznych (co odpowiada wymiarowi bazowej przestrzeni konfiguracji $\dim \mathcal{C}_4 = 10$):

\begin{equation}
E_{\text{bounded}} = \frac{1}{2} \sum E_i \implies 10 \text{ niezależnych więzów topologicznych}
\end{equation}

Ostatecznie otrzymujemy zredukowany wymiar fizycznych stopni swobody:
\begin{equation}
\dim G_{\text{int}}^{\text{reduced}} = 22 - 10 = 12
\end{equation}
...
Zgodność algebraiczna wymiarów (12 wymiarów) stanowi dowód istnienia pojemności przestrzeni na osadzenie Modelu Standardowego. Zauważamy, że istnieje zaledwie kilka nieizomorficznych 12-wymiarowych algebr Liego, z których dekompozycja w produkt prosty podgrup $SU(3) \times SU(2) \times U(1)$ jest najbardziej stabilna termodynamicznie w obecności potencjału Higgsa wynikającego z Fluktuacji Brzegowych (dowód topologii generatorów algebry Liego $f^{abc}$ pozostawiamy do kolejnych publikacji jako otwarty problem relacji komutacji).
...
Wniosek: Dla fizycznego wymiaru $d=4$ redukcja więzów narzuca dokładnie 12 wewnętrznych stopni swobody. Otwiera to bezpośrednią, kombinatoryczną drogę na wbudowanie grupy symetrii $SU(3) \times SU(2) \times U(1)$ bez sztucznego zakładania jej istnienia a priori. $\square$
...
\textbf{Wniosek i Twierdzenie o Stochastycznej Emergencji Lorentza:} Próby budowania dyskretnych krat przestrzennych klasycznie obarczone są problemem łamania symetrii Lorentza (efekt wybranego układu odniesienia). W teorii SHZ-U sieć horyzontów nie jest jednak krystalicznym, sztywnym układem punktów, lecz kwantowo superponowaną, stochastyczną siecią relacyjną. Kwantowe uśrednianie topologii po wszystkich możliwych konfiguracjach grafu sprawia, że ewentualne mikroskopijne zjawiska anizotropowe ulegają wzajemnej kasacji (destruktywna interferencja). W efekcie makroskopowa symetria Lorentza wyłania się w granicy ciągłej jako ścisła izotropia przestrzeni, a dyspersja prędkości światła z racji zjawisk ziarnistości leży poza obecną granicą detekcji ($\delta c/c < 10^{-32}$). $\square$
...
\textbf{Krok 1: Globalna Hermitowskość a Lokalna "Utrata" Energii}\\

Hamiltonian całkowity układu:
\begin{equation}
H_{\text{SHZ}}^\dagger = \sum_{\langle i,j \rangle} \frac{\hbar\omega_0}{2}(a_j^\dagger a_i + a_i^\dagger a_j) + V_{\text{int}}^\dagger = H_{\text{SHZ}}
\end{equation}
jest jawnie hermitowski, co gwarantuje unitarność ewolucji $U(t) = e^{-iH_{\text{SHZ}}t/\hbar}$. Pozorny paradoks wynikający z "utraty" połowy energii topologicznej na złączu horyzontów zostaje rozwiązany przez fakt, że energia ta nie opuszcza izolowanego układu przestrzeni Hilberta. W warunkach globalnej ewolucji unitarnej transformuje się ona w zmagazynowaną energię kondensatu (VEV) próżni zawartą w członie oddziaływań $V_{\text{int}}$. Zatem mechanika kwantowa zostaje w pełni zachowana na poziomie Wszechświata, a "dyssypacja" zachodzi jedynie między podukładami.

\bigskip

\textbf{Krok 2: Lokalność Topologiczna a Makroskopowa Przyczynowość}\\

$H_{\text{SHZ}}$ jest sumą operatorów działających na topologicznie sąsiednie węzły grafu:
\begin{equation}
H_{\text{SHZ}} = \sum_{\langle i,j \rangle} H_{ij}
\end{equation}

gdzie $H_{ij}$ nie zawiera operatorów z węzłów $k \notin \{i,j\}$. Na poziomie sub-planckowskim definiuje to ścisłą \textit{lokalność topologiczną} (brak sprzężeń pozadiagonalnych dla nielokalnych krawędzi). Makroskopowa lokalność czasoprzestrzenna, rozumiana w sensie OTW, jest tu zjawiskiem emergentnym: sama gładka metryka oraz stożki świetlne definiujące zasady przyczynowości wyłaniają się dopiero w granicy termodynamicznej (kontinuum) jako uśrednienie tych dyskretnych, topologicznie ograniczonych relacji.
